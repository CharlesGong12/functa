...
class LatentModulatedSiren(nn.Layer):
    ...
    def __init__(self,
        ...
        meta_sgd_init_range: Tuple[float, float] = (1e-7, 5e-4),
        ...
        for i in range(depth):
            # modulate_scale=False,modulate_shift=False,apply_activation=False
            # so only a linear is applied
            siren_layers.append(
            ...
            # self.bn.append(nn.BatchNorm1D(num_features=width if i != depth - 1 else self.out_channels,data_format='NLC'))
...
class ModulatedSiren(nn.Layer):
    ...
    def __init__(self,
        ...
        meta_sgd_clip_range: Tuple[float, float] =(1e-8, 5.) ):   #(-5., 5.)
        """Constructor.
